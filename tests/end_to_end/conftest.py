...
@pytest.fixture
def gh_repo(gh_me, git_repo, clean_gh_after_test):
    # TODO: should this depend on request.node.name ?
    name = "end_to_end_python_coverage_comment_action"
    ...
    if clean_gh_after_test:
        gh_me("repo", "delete", name, "--confirm")


@pytest.fixture
def gh_fork(cd, gh_other, git_repo, clean_gh_after_test):
    cd("fork")
    gh_other("repo", "fork")
